...
class Train_Predict:
    ...
    def __init__(self, train_df, test_df, feature, criterion, sampling_p, hidden_layers, lr=6e-4, weight_decay=0):
    ...
    def init_model(self):
        self.model_trainer = Model_Trainer(
            output_size=4, feature=self.feature, 
    ...
    def make_off_df(self, epoch_num, k):
        ...
        for trn, val in tqdm( k.split(self.train_df, self.train_df.jobflag), total=k.n_splits ):
            ...
            trn_dataloader = DataLoader(
                trn_data_set, shuffle=False, batch_size=512,
                 sampler=ImbalancedDatasetSampler(trn_data_set, sampling_type=self.sampling_p)
                 )
    ...
    def predict_test_df(self, epoch_num):
        ...
        trn_dataloader = DataLoader(
            trn_data_set, shuffle=False, batch_size=512,
            sampler=ImbalancedDatasetSampler(trn_data_set, self.sampling_p)
            )
